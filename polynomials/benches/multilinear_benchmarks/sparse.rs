...
fn sample_poly() -> SparseMultilinearPolynomial<Fq> {
    let mut rng = rand::thread_rng();
    let mut terms = Vec::with_capacity(100);

    for i in 0..100 {
        terms.push((Fq::rand(&mut rng), i));
    }

    SparseMultilinearPolynomial::new(terms, 100)
}
fn sample_poly_2() -> SparseMultilinearPolynomial<Fq> {
    let mut rng = rand::thread_rng();
    let mut terms = Vec::with_capacity(100);
...
pub fn sparse_multilinear_polynomial_benchmarks(c: &mut Criterion) {
    ...
    //         }
    //         black_box(SparseMultilinearPolynomial::interpolate(xs, ys))
    //     });
    // });
}
